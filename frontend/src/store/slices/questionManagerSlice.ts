...
export const queryQuestions = createAsyncThunk(
  'queryQuestions',
  async (
    params: { examId: string; pageIndex: number; pageSize: number },
    { dispatch, rejectWithValue }
  ) => {
    dispatch(loaderActions.toggleLoading());

    try {
      const response = await axios.get(`/exam/${params.examId}/question`, {
        params,
      });
...
export const deleteQuestion = createAsyncThunk(
  ...
  async ({ examId, _id }: Question, { dispatch, rejectWithValue }) => {
    ...
    try {
      const response = await axios.delete(`/exam/${examId}/question/${_id}`);
      return response.data;
    } catch (err) {
      const errorMessage = axios.isAxiosError(err)
        ? (err as AxiosError<{ message: string }, any>).response!.data.message
